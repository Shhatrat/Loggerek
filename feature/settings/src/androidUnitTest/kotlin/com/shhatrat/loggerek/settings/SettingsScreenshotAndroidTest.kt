...
class SettingsScreenshotAndroidTest {
    ...
    fun authScreenWithError() {
        screenshotTestSource.settingsScreen().forEach { item ->
    ...
    fun authScreenWithUser() {
        screenshotTestSource.realOnStart().forEach { item ->
            runIntroScreenTest(item.content, item.description)
        }
        ...
        runComposeUiTest {
            ...
            onRoot().captureRoboImage(
                DefaultFileNameGenerator.generateFilePath().addPackagePrefix(
                    Type.ANDROID,
                    this@SettingsScreenshotAndroidTest::class.java.packageName
